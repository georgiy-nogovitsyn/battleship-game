...
from pygame_utils import highlight_cell, get_highlighted_cell_coordinate, draw_basic_board, draw_ships, draw_water_shots, draw_board_numbers
...
def draw_pygame_elements():
    ...
    draw_water_shots(screen, player.board.field)
    draw_water_shots(screen, player.board.battlefield, offset_x=right_border_offset)
...
while running:
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN and mouse_last_pos:
                ...
                if events.chosen_coordinate_validation(mouse_last_pos, player, comp) is True:
                    if events.player.attack(mouse_last_pos, comp) is True:
                        print(f'You hit the ship on {mouse_last_pos}')
                        if events.ships_status_update(comp.ships) is True:
                            winner = 'Player'
                            running = False
                            break
                    else:
    ...
    while player_2_turn:
        ...
        if comp.attack(computer_choice_coordinates, player):
            ...
            if events.ships_status_update(player.ships) is True:
                winner = 'Computer'
                running = False
        else:
